  ...
  if (data.substr(0, 5) == "!clip") {
    ...
    if (data[5] == ' ') {
      ...
      if (content.size() > m_message_size) {
        IRCMessage msg{
            "PRIVMSG",
            {name, std::string{"Your message is too big. Limit is "} +
      ...
      IRCMessage msg{"PRIVMSG", {name, "This is your clipboard:"}};
      send(msg);
      ...
      IRCMessage msg{"PRIVMSG",
                     {name,
                      "Commands: !clip - save to clipboard,"
      ...
      if (index < static_cast<int>(clipboard.size())) {
        ...
        IRCMessage msg{"PRIVMSG", {name, "Index is too large!"}};
...
void Clipboard::onNewConfiguration() {
  m_size = getConfig()["config"]["size"].as<size_t>();
  m_message_size = getConfig()["config"]["message_size"].as<size_t>();
}
